...
def test_instant_order_atomicity_rollback(db_session, client, monkeypatch):
    """
    Test atomicity rollback: Mock process_payment to raise an exception.
    Verify that the order, invoice, and ledger records are fully rolled back,
    and inventory is not changed.
    """
    from app.models.invoice import Invoice
    ...
    cust = Customer(tenant_id=tenant.id, retailer_name="Atomicity Cust", credit_limit=50000.0)
    prod = Product(tenant_id=tenant.id, sku_id="PROD-ATOM-1", base_price=100.0, brand="Generic", category="Grocery", pack_size="1 unit")
    db_session.add_all([cust, prod])
    db_session.commit()

    inv = Inventory(tenant_id=tenant.id, sku_id=prod.id, quantity_on_hand=10, quantity_committed=0, location="Aisle-A1")
    db_session.add(inv)
    db_session.commit()

    # Mock process_payment to fail
    def mock_process_payment(*args, **kwargs):
        raise ValueError("Mocked payment processing failure")
    
    import app.services.payment_service
    monkeypatch.setattr("app.services.payment_service.process_payment", mock_process_payment)
    ...
    payload = {
        ...
        "source": "Portal",
        "status": "Delivered",
        "items": [{"sku_id": "PROD-ATOM-1", "quantity": 3, "unit_price": 100.0}],
        "already_delivered": True,
        "payment_method": "CASH",
        "cash_amount_received": 300.0,
        "idempotency_key": str(uuid.uuid4())
    }

    # Verify exception is raised and transaction rolls back
    import pytest
    with pytest.raises(ValueError, match="Mocked payment processing failure"):
        client.post("/api/v1/orders", json=payload)
    
    # Verify no order, invoice or ledger entries were created
    db_session.expire_all()
    assert db_session.query(Order).filter_by(customer_id=cust.id).count() == 0
    assert db_session.query(Invoice).filter_by(customer_id=cust.id).count() == 0
    assert db_session.query(CustomerLedger).filter_by(customer_id=cust.id).count() == 0
    
    # Verify inventory remains untouched
    inv_check = db_session.get(Inventory, inv.id)
    assert inv_check.quantity_on_hand == 10
    assert inv_check.quantity_committed == 0


def test_instant_order_credit_limit_boundaries(db_session, client):
    """
    Test credit limit check boundaries:
    - Exactly equal to credit limit succeeds.
    - Exactly 1 over credit limit fails.
    """
    ...
    # Setup customer with 500.0 credit limit
    cust = Customer(tenant_id=tenant.id, retailer_name="Credit Boundary Cust", credit_limit=500.0)
    prod = Product(tenant_id=tenant.id, sku_id="PROD-CRED-1", base_price=100.0, brand="Generic", category="Grocery", pack_size="1 unit")
    db_session.add_all([cust, prod])
    db_session.commit()
    ...
    # Case A: Exactly equal to credit limit (500.0)
    payload_a = {
        "tenant_id": str(tenant.id),
        "customer_id": str(cust.id),
        "source": "Portal",
        "status": "Delivered",
        "items": [{"sku_id": "PROD-CRED-1", "quantity": 5, "unit_price": 100.0}],
        "already_delivered": True,
        "payment_method": "CREDIT",
        "idempotency_key": str(uuid.uuid4())
    }
    response = client.post("/api/v1/orders", json=payload_a)
    assert response.status_code == 201
    
    db_session.expire_all()
    
    # Case B: Exactly 1 over credit limit (Order total 501.0, limit is 500.0 but outstanding is already 500.0)
    # Available credit is now 0.0. An order of total 1.0 will exceed it.
    payload_b = {
        "tenant_id": str(tenant.id),
        "customer_id": str(cust.id),
        "source": "Portal",
        "status": "Delivered",
        "items": [{"sku_id": "PROD-CRED-1", "quantity": 1, "unit_price": 1.0}],
        "already_delivered": True,
        "payment_method": "CREDIT",
        "idempotency_key": str(uuid.uuid4())
    }
    response2 = client.post("/api/v1/orders", json=payload_b)
    assert response2.status_code == 400
    assert "Credit limit exceeded" in response2.json()["detail"]


def test_instant_order_cash_consistency(db_session, client):
    """
    Verify that cash payment recorded via the new instant order flow produces
    identical ledger and outstanding balance state as recording the standard order
    and then recording payment voucher via Collections manual endpoint.
    """
    from app.models.invoice import Invoice
    from app.models.ledger import CustomerLedger

    tenant = DistributorTenant(name="Consistency Tenant")
    db_session.add(tenant)
    db_session.commit()
    tenant_context.set(tenant.id)

    # Setup two identical customers and products
    cust_a = Customer(tenant_id=tenant.id, retailer_name="Cust A", credit_limit=50000.0)
    cust_b = Customer(tenant_id=tenant.id, retailer_name="Cust B", credit_limit=50000.0)
    prod = Product(tenant_id=tenant.id, sku_id="PROD-CONSIST", base_price=100.0, brand="Generic", category="Grocery", pack_size="1 unit")
    db_session.add_all([cust_a, cust_b, prod])
    db_session.commit()

    inv = Inventory(tenant_id=tenant.id, sku_id=prod.id, quantity_on_hand=50, quantity_committed=0, location="Aisle-A1")
    db_session.add(inv)
    db_session.commit()

    # Flow A: Instant order creation + cash payment received inline
    payload_a = {
        "tenant_id": str(tenant.id),
        "customer_id": str(cust_a.id),
        "source": "Portal",
        "status": "Delivered",
        "items": [{"sku_id": "PROD-CONSIST", "quantity": 10, "unit_price": 100.0}],
        "already_delivered": True,
        "payment_method": "CASH",
        "cash_amount_received": 1000.0,
        "reference_number": "REF-CONSIST-A",
        "idempotency_key": str(uuid.uuid4())
    }
    response_a = client.post("/api/v1/orders", json=payload_a)
    assert response_a.status_code == 201

    # Flow B: Standard order creation confirmed, followed by manual Collections payment
    payload_b = {
        "tenant_id": str(tenant.id),
    ...
    response_b = client.post("/api/v1/orders", json=payload_b)
    assert response_b.status_code == 201
    
    # Record payment voucher via manual Collections voucher endpoint
    invoice_b = db_session.query(Invoice).filter(Invoice.customer_id == cust_b.id).one()
    voucher_payload = {
        "customer_id": str(cust_b.id),
        "amount": 1000.0,
        "method": "CASH",
    ...
    voucher_response = client.post(f"/api/v1/payments/collection-voucher?tenant_id={tenant.id}", json=voucher_payload)
    assert voucher_response.status_code == 201

    # Fetch fresh db state
    db_session.expire_all()
    c_a = db_session.get(Customer, cust_a.id)
    c_b = db_session.get(Customer, cust_b.id)
    
    # Assert outstanding balance is identical
    assert float(c_a.outstanding_balance) == float(c_b.outstanding_balance)

    # Assert ledger entries count and types are identical
    ledgers_a = db_session.query(CustomerLedger).filter_by(customer_id=cust_a.id).order_by(CustomerLedger.created_at.asc()).all()
    ledgers_b = db_session.query(CustomerLedger).filter_by(customer_id=cust_b.id).order_by(CustomerLedger.created_at.asc()).all()
    
    assert len(ledgers_a) == len(ledgers_b)
    for la, lb in zip(ledgers_a, ledgers_b):
        assert la.type == lb.type
        assert float(la.amount) == float(lb.amount)


def test_instant_order_idempotency_replay(db_session, client):
    """
    Verify idempotency key replay: submitting same request with same idempotency_key
    twice (simulating network retry) returns the same order ID and doesn't double-create.
    """
    from app.models.invoice import Invoice
    from app.models.payment import Payment

    tenant = DistributorTenant(name="Idempotency Replay Tenant")
    db_session.add(tenant)
    db_session.commit()
    tenant_context.set(tenant.id)

    # Setup customer and product
    cust = Customer(tenant_id=tenant.id, retailer_name="Idempotency Cust", credit_limit=50000.0)
    prod = Product(tenant_id=tenant.id, sku_id="PROD-IDEMP-1", base_price=100.0, brand="Generic", category="Grocery", pack_size="1 unit")
    db_session.add_all([cust, prod])
    db_session.commit()

    inv = Inventory(tenant_id=tenant.id, sku_id=prod.id, quantity_on_hand=20, quantity_committed=0, location="Aisle-A1")
    db_session.add(inv)
    db_session.commit()
    ...
    payload = {
        ...
        "source": "Portal",
        "status": "Delivered",
        "items": [{"sku_id": "PROD-IDEMP-1", "quantity": 2, "unit_price": 100.0}],
        "already_delivered": True,
        "payment_method": "CASH",
        "cash_amount_received": 200.0,
        "idempotency_key": str(key)
    }

    # First request
    response_1 = client.post("/api/v1/orders", json=payload)
    assert response_1.status_code == 201
    data_1 = response_1.json()
    order_id_1 = data_1["order_id"]

    # Second duplicate request (simulated replay)
    response_2 = client.post("/api/v1/orders", json=payload)
    assert response_2.status_code == 201  # Should return 201 (success)
    data_2 = response_2.json()
    order_id_2 = data_2["order_id"]

    assert order_id_1 == order_id_2
    assert data_2.get("is_duplicate") is True

    # Verify only one order, one invoice, and one payment exist in DB
    db_session.expire_all()
    assert db_session.query(Order).filter_by(customer_id=cust.id).count() == 1
    assert db_session.query(Invoice).filter_by(customer_id=cust.id).count() == 1
    assert db_session.query(Payment).filter_by(customer_id=cust.id).count() == 1

    # Verify inventory was only decremented once
    inv_check = db_session.get(Inventory, inv.id)
    assert inv_check.quantity_on_hand == 18
    assert inv_check.quantity_committed == 0
