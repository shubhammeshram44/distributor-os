...
export default function CustomersPage() {
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/v1/customers?${params}`, {
        credentials: "include",
        headers: {
  ...
  const handleExport = async () => {
    try {
      const response = await fetch(
        `${apiBase}/api/v1/customers/export?tenant_id=${tenantId}`,
        {
          credentials: "include",
  ...
  const handleDeleteCustomer = async () => {
    ...
    try {
      const response = await fetch(`${apiBase}/api/v1/customers/${confirmDialog.customerId}`, {
        method: "DELETE",
        credentials: "include",
        headers: token ? { "Authorization": `Bearer ${token}` } : {}
      });
      ...
      if (response.ok) {
        ...
        setConfirmDialog({ isOpen: false, customerId: "", action: "" });
      }
    } catch (err) {
  ...
  return (
    <ErrorBoundary>
      <div className="flex bg-dashboard-bg min-h-screen text-slate-800">
        <Sidebar activeTab="Customers" setActiveTab={() => {}} tenantName="Workspace" />
          ...
          <main className="flex-1 mt-16 p-6 overflow-y-auto">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-slate-400 font-semibold mt-1">Manage your customer relationships</p>
              </div>
            ...
            {loading ? (
              ...
              <EmptyState
                ...
                description="Start by adding your first customer or adjust your filters"
                customIcon="customers"
              />
            ) : (
              <>
                    ...
                    <table className="w-full">
                      <thead className="border-b border-dashboard-border bg-slate-50">
                        <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-bold text-slate-600 uppercase">Contact</th>
                        ...
                        {customers.map((customer) => (
                          <tr key={customer.id} className="hover:bg-slate-50 transition-colors">
                            <td className="px-6 py-4">
                              ...
                              <p className="text-xs text-slate-500 mt-0.5">ID: {customer.id.slice(0, 8)}</p>
                            </td>
                            <td className="px-6 py-4">
        ...
        <ConfirmDialog
          ...
          onCancel={() => setConfirmDialog({ isOpen: false, customerId: "", action: "" })}
        />
      </div>
    </ErrorBoundary>
  );
